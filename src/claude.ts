...
// Opus 5 gives the best judgement on match quality, which is the whole product.
// If per-CV cost becomes the binding constraint, "claude-sonnet-5" is the step
// down to measure against — do that with an eval, not a hunch.
...
const ASSESSMENT_TOOL: Anthropic.Tool = {
  ...
  description:
    "Record the structured assessment of one candidate against the job spec. Call this exactly once.",
  strict: true,
  input_schema: {
    type: "object",
...
Always call the record_assessment tool exactly once. Do not reply with prose.`
...
export async function screenCandidate(jobSpec: string, cvText: string): Promise<Assessment> {
  const response = await client.messages.create({
    ...
    system: [
      // The spec repeats across every CV in a batch, so cache it.
      { type: "text", text: SCREEN_SYSTEM },
      {
        type: "text",
        text: `<job_spec>\n${jobSpec}\n</job_spec>`,
    ...
    messages: [
      {
        role: "user",
        content: `Assess this candidate against the job spec above.\n\n<cv>\n${cvText}\n</cv>`,
      },
    ],
  })
  ...
  const call = response.content.find(
    (block): block is Anthropic.ToolUseBlock =>
      block.type === "tool_use" && block.name === "record_assessment",
  )
  if (!call) {
    throw new Error(
      "The model did not return an assessment. Check that the CV extracted readable text, then try again.",
    )
  }
  return call.input as Assessment
}
